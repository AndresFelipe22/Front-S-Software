...
import { Button, dividerClasses, Menu, MenuItem, styled } from '@mui/material';
...
export default function OrderTable() {
  ...
  const handleUpdateOrderStatus=(orderId:number, orderStatus:any)=>() =>{
    dispatch(updateOrderStatus({jwt:localStorage.getItem("jwt") || "" , orderId,
      orderStatus}))
  }
    ...
    <StyledTableRow key={item.id}>
          ...
          <div className='flex gap-1 flex-wrap'>
            {
              ...
              '>
                ...
                <div className='flex flex-col justify-between py-2'>
                  <h1>Titulo: {orderItem.product.title}</h1>
                  <h1>Precio: {orderItem.product.sellingPrice}</h1>
        ...
        <StyledTableCell align="right">
          <div className='flex flex-col gap-y-2'>
                    ...
                    <h1><strong>Movil:</strong> {item.shippingAddress.mobile}</h1>
          </div>
        </StyledTableCell>
        <StyledTableCell align="right">
          <span className='px-5 py-2 border rounded-full
